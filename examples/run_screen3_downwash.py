...
# This set takes not long at all
...
# This set takes pretty long (1520 runs!) but facilitates pretty nice figures
...
n_desired = 10
n_tot = HB_vals.size
n_stride = np.floor(n_tot/n_desired).astype(int) if n_tot > n_desired else None
...
# %% Examine both params for one position
...
for c, X_choice in enumerate(X_choices):
    ...
    im = ax.pcolormesh(HW_vals, HB_vals, data, shading="auto")
